...
## NOTE: This file contains a direct copy of the popsize file found in the
## drpop package. The CRAN mirror can be found here:
## https://github.com/cran/drpop/blob/master/R/popsize.R
## We've decided we want to implement this in a different manner, but take
## inspiration from this package.
        ...
        result <- psiinv %>%
            mutate(
                ...
                cin.l = round(pmax(
                    N *
                        psiin -
                        1.96 * sqrt(N^2 * sigma^2 + N * psiin * (psiin - 1)),
                    N
                )),
                cin.u = round(
                    N *
            ...
            for (k in setdiff(list2_vec, list1_vec[list1_vec <= j0])) {
                ...
                for (folds in 1:nfolds) {
                    if (nfolds == 1) {
                        ...
                        sbset = ((folds - 1) * ceiling(N / nfolds) + 1):(folds *
                            ceiling(N / nfolds))
                        sbset = sbset[sbset <= N]
...
#' Readable companion implementation of popsize_base for side-by-side
#' comparison. This function is intentionally not wired into any existing
#' workflow and does not replace popsize_base.
#'
#' @keywords internal
#' @noRd
popsize_base_refactor <- function(
    data,
    K = 2,
    j0,
    k0,
    ...
    # Number of observed/captured units and number of covariates.
    N <- nrow(data)
    l <- ncol(data) - K
    stopifnot(N > 1)
    ...
    list1_vec <- if (!missing(j0)) j0 else c(1:(K - 1))
    list2_vec <- if (!missing(k0)) k0 else c(1:K)
    # This is just a combination of lists
    build_listpairs <- function(list1_vec, list2_vec) {
        return(unlist(sapply(list1_vec, function(j1) {
            sapply(
                setdiff(list2_vec, list1_vec[list1_vec <= j1]),
                function(k1) {
                    return(paste(min(j1, k1), ",", max(j1, k1), sep = ""))
    ...
    listpair <- build_listpairs(list1_vec = list1_vec, list2_vec = list2_vec)

    # if there are no covariates do petersen estimate
    if (l == 0) {
        colnames(data) <- c(paste("L", 1:K, sep = ""))
        ...
        for (j in list1_vec) {
            j_anchor <- j
            # validate they're binary columns again
            if (!setequal(data[, j], c(0, 1))) {
                next
            }

            for (k in setdiff(list2_vec, list1_vec[list1_vec <= j_anchor])) {
                # another validation check for binary columns
                if (!setequal(data[, k], c(0, 1))) {
                    next
                }

                # Not possible assuming list1_vec is 1:K-1 and list2_vec is 1:K
                # because list1_vec will filter out j_anchor and lower from
                # list2_vec in line 1094
                if (j_anchor > k) {
                    j <- k
                    k <- j_anchor
                } else {
    ...
    # Doubly-robust branch (l > 0): nuisance estimation + cross-fitting.
    colnames(data) <- c(
        paste("L", 1:K, sep = ""),
        paste("x", 1:(ncol(data) - K), sep = "")
    )
    ...
    summarize_results <- function(psiinv_summary, var_summary, N) {
        return(list(
            psi = 1 / psiinv_summary,
            sigma = sqrt(N * var_summary),
            n = round(N * psiinv_summary),
